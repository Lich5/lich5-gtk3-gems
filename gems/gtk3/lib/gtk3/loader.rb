...
module Gtk
  class Loader < GObjectIntrospection::Loader
    ...
    def require_extension
      ...
      # For Windows binary gems, bundled DLLs are in lib/gtk3/vendor/bin/. We must add this
      # to the DLL search path BEFORE loading gtk3.so, otherwise Windows won't find the
      # required GTK3 DLLs and loading will fail with LoadError 126.
      if Gem.win_platform?
        vendor_bin = File.expand_path(File.join(__dir__, '..', 'gtk3', 'vendor', 'bin'))
      ...
      # .so files in version-specific directories: lib/gtk3/3.3/gtk3.so, lib/gtk3/3.4/gtk3.so
